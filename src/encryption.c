...
void uem256_hash(const unsigned char *input, size_t length, unsigned char output[32]) {
    uint64_t state[4] = {
        0x6d6574615f6d6172ULL,
        0x7465735f7169735fULL,
        0x6e69755f4d654f6dULL,
        0x646f6d5f656e6f6dULL
    };

    size_t offset = 0;
    while (offset + 8 <= length) {
        uint64_t block = load64_le(input + offset);
        uem256_mix(state, block);
        offset += 8;
    }

    unsigned char tail[8] = {0};
    size_t remaining = length - offset;
    memcpy(tail, input + offset, remaining);
    tail[remaining] = 0x80;
    tail[7] = (unsigned char)length;
    uem256_mix(state, load64_le(tail));

    state[0] ^= (uint64_t)length;
    for (int i = 0; i < 4; ++i) {
        uem256_mix(state, state[i] ^ 0xFEDCBA9876543210ULL);
    }

    for (int i = 0; i < 4; ++i) {
        store64_le(state[i], output + 8 * i);
    }
}
